...
**Responsable de la base de datos:** {{titular}}, CUIT {{cuit}}, con domicilio en {{domicilio}}.
...
un usuario cargó, escribinos a {{email}}. Daremos intervención al
...
**Podés oponerte** escribiendo a {{email}}, sin que ello afecte tu acceso
al Servicio.
...
| **Google** (modelos Gemini) *(activo sólo si ese proveedor está configurado)* | Lo mismo que el anterior, cuando el Asistente opera con este proveedor | Estados Unidos |
| **OpenAI** *(activo sólo si ese proveedor está configurado)* | **El audio de tu micrófono** para transcripción y el texto de las respuestas para generar voz. En el modo de conversación por voz en tiempo real recibe además: **(a)** tu dirección IP, porque la conexión se establece directamente desde tu navegador; **(b)** la **transcripción completa de la conversación**; y **(c)** la memoria que el asistente de voz guardó sobre vos y **los datos de tus proyectos que consulta para responderte**, que pueden incluir nombres de clientes o inversores y montos, conforme al punto 3.2 | Estados Unidos |
...
| **Resend** | Tu correo, tu nombre y el contenido de los mensajes transaccionales (códigos de verificación, enlaces de recuperación) | Según la política del proveedor |
...
| **Google Maps** | Las direcciones que le indicás al Asistente para optimizar recorridos | Según la política del proveedor |
| **Tavily** | El texto de la búsqueda que el sistema construye a partir de tu consulta y, para precios de materiales, tu zona | Según la política del proveedor |
| **Telegram** | Tu identificador de chat y el contenido de los avisos que pidas recibir por ese canal. **Si además activás la conversación con el Asistente por Telegram**, el contenido de los mensajes que le escribas y de sus respuestas | Según la política del proveedor |
...
de ubicaciones, escribinos a {{email}}: el control autoservicio está
...
| Qué | Por qué | Plazo |
|---|---|---|
| **Documentos generados** (informes PDF/planillas) alojados en el almacenamiento de archivos | Se generan sin vínculo con tu cuenta, por lo que técnicamente no pueden identificarse para su borrado selectivo | Según la política del proveedor |
| **Copias de seguridad** | Rotación técnica del proveedor de base de datos | Según el plan contratado con el proveedor |
| **Registros técnicos (logs)** | Diagnóstico y seguridad. **No registramos tu correo al purgar la cuenta** | Según retención del proveedor de hosting Según la política del proveedor |
...
El Servicio está dirigido a personas mayores de 18
...
**Responsable:** {{titular}}
**Correo para privacidad y derechos:** {{email}}
