...
## 🔎 Diagnóstico actualizado (29-jul): NO es la migración

Descartamos la hipótesis de Alembic con dos evidencias duras:

1. **El último commit desplegado (`4704d42`) NO tocó backend.** Cambió solo
   `frontend/app.css` y `frontend/app.html`. O sea que Railway **no tiene watch
   paths de `backend/**`** (si los tuviera, ese commit no habría disparado
   deploy). Despliega cualquier commit... y desde el 25-jul no desplegó
   **ninguno de los 9** que siguieron.
2. **`8ffe7af` (Almacenamiento) no trae ninguna migración** — son rutas nuevas y
   nada más. Igual no se desplegó. Si el `preDeployCommand` fuera el problema,
   ese commit habría entrado sin drama.

**Conclusión: el deploy ni siquiera arranca.** El sospechoso pasa a ser la
conexión repo↔servicio: auto-deploy pausado, GitHub App desautorizada, servicio
despegado del repo, o la branch de deploy apuntando a otro lado. Nada de eso se
ve ni se arregla desde el código — es todo dashboard.

**Lo que necesitamos de vos** (3 minutos en Railway):
1. **Settings → Source**: ¿el servicio sigue conectado al repo? ¿La branch es
   `main`? ¿"Auto Deploy" está en ON? ¿Hay watch paths cargados?
2. **Deployments**: ¿aparece ALGÚN intento después del 25-jul? Si no aparece
   ninguno, es 100% conexión (no build ni migración). Si aparecen en Failed,
   mandanos el log del pre-deploy.
3. **Dale "Redeploy" / "Deploy latest commit"** sobre `main`. Si entra, listo:
   se destraban las 3 features de una.
4. Si el servicio quedó despegado, reconectalo al repo y volvé a deployar.

> Todo lo nuestro ya está verificado offline: `alembic upgrade --sql` corre
> limpio (0036 con IDENTITY, 0037 son 11 `ADD COLUMN` nullables), `main.py`
> importa sin errores y las suites pasan. No hay nada más que podamos hacer de
> este lado.

**Mientras tanto** (`0858d36`+): las secciones nuevas ya no muestran un error
rojo genérico. Detectan el 404 y avisan *"todavía no está disponible, falta
publicar la última versión del servidor, tus datos están intactos"*, así el
usuario no cree que se le rompió la cuenta.

**Hipótesis vieja (ya descartada, se deja como registro)**: se creía que el
`preDeployCommand: alembic upgrade head` fallaba por la migración 0036, que era
la única que emitía `CREATE SEQUENCE` + `ALTER SEQUENCE ... OWNED BY` sueltos.
Se corrigió igual en `f85b4bf` (ahora usa `GENERATED BY DEFAULT AS IDENTITY`) y
está bien así, pero no era la causa.

> Si preferís no debuggear: pasanos el log del pre-deploy y lo resolvemos
> nosotros. Lo único que no podemos hacer es verlo (el dashboard es tuyo).
